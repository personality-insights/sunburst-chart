...
class PersonalitySunburstChart {
  ...
  constructor(options) {
    ...
    this.d3Container = d3.select('#' + this.containerId),
  ...
  show(theProfile, personImageUrl) {
    const self = this;
    const d3Profile = self.version === 'v3' ? new D3PersonalityProfileV3(theProfile) : new D3PersonalityProfileV2(theProfile);
    const widgetId = self.containerId;
